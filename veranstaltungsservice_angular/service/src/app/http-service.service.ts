...
export class HttpServiceService {
  ...
  // end-point url
  base_url: string = 'http://35.246.170.94:8000/api/';
  ...
  public login(user) {
    this.http.post('http://35.246.170.94:8000/api-token-auth/', JSON.stringify(user), this.httpOptions).subscribe(
      data => {
  ...
  public refreshToken() {
    this.http.post('http://35.246.170.94:8000/api-token-refresh/', JSON.stringify({token: this.token}), this.httpOptions).subscribe(
      data => {
        this.updateData(data['token']);
      },
      err => {
